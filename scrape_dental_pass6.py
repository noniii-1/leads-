# -*- coding: utf-8 -*-
"""
Sixth pass: needed because the recency filter (<=6 months) cuts the qualifying
pool roughly in half, so more raw volume is needed to still reach 140 new leads
that pass ALL three criteria (no website, decent traffic/rating, recent review).
"""
import time
import json
        ...
        try:
            ...
            for a in articles:
                ...
                if rec["reviews"] >= MIN_REVIEWS and not rec["has_website"]:
                    ...
                    qualifying_count += 1
                    new_this_query += 1
            print(f"[dental-p6] [{gi}] '{query}' -> {len(articles)} items, +{new_this_query} qualifying (total {qualifying_count})", flush=True)
        except Exception as e:
            print(f"[dental-p6] [{gi}] '{query}' ERROR: {e}", flush=True)
        time.sleep(0.6)


def main():
    queries = build_queries()
    print(f"[dental-p6] {len(queries)} queries total (global index shown)", flush=True)

    CHUNK = 40
    for start in range(0, len(queries), CHUNK):
        chunk = queries[start:start + CHUNK]
        print(f"[dental-p6] opening fresh session for global queries {start+1}-{start+len(chunk)}", flush=True)
        try:
            with StealthySession(headless=True, network_idle=False, max_pages=1) as session:
                run_batch(chunk, session, start + 1)
        except Exception as e:
            print(f"[dental-p6] session-level ERROR: {e}", flush=True)

    print("[dental-p6] DONE ALL", flush=True)
