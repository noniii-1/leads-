# -*- coding: utf-8 -*-
"""
Fourth (likely final) pass: last fresh keyword variants, same 33 comunas,
same MIN_REVIEWS=15. Appends to dental_candidates.jsonl.
"""
import time
import json
...
KEYWORDS = [
    "clinica dental sonrisas", "consulta odontologica", "especialista dental",
    "salud oral clinica",
]
        ...
        try:
            ...
            for a in articles:
                ...
                if rec["reviews"] >= MIN_REVIEWS and not rec["has_website"]:
                    ...
                    qualifying_count += 1
                    new_this_query += 1
            print(f"[dental-p4] [{gi}] '{query}' -> {len(articles)} items, +{new_this_query} qualifying (total {qualifying_count})", flush=True)
        except Exception as e:
            print(f"[dental-p4] [{gi}] '{query}' ERROR: {e}", flush=True)
        time.sleep(0.6)


def main():
    queries = build_queries()
    print(f"[dental-p4] {len(queries)} queries total (global index shown)", flush=True)

    CHUNK = 40
    for start in range(0, len(queries), CHUNK):
        chunk = queries[start:start + CHUNK]
        print(f"[dental-p4] opening fresh session for global queries {start+1}-{start+len(chunk)}", flush=True)
        try:
            with StealthySession(headless=True, network_idle=False, max_pages=1) as session:
                run_batch(chunk, session, start + 1)
        except Exception as e:
            print(f"[dental-p4] session-level ERROR: {e}", flush=True)

    print("[dental-p4] DONE ALL", flush=True)
