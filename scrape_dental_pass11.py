# -*- coding: utf-8 -*-
"""
Eleventh pass: closing the last gap to 140. MIN_REVIEWS=10, fresh keywords.
"""
import os
import time
import json
...
def _watchdog():
    while True:
        ...
        if stale > WATCHDOG_SECS:
            print(f"[dental-p11] WATCHDOG: no progress for {stale:.0f}s, force-exiting", flush=True)
            os._exit(1)
...
KEYWORDS = [
    "dentista particular", "salud dental clinica",
    "policlinico dental", "protesis dental",
]
        ...
        try:
            ...
            for a in articles:
                ...
                if rec["reviews"] >= MIN_REVIEWS and not rec["has_website"]:
                    ...
                    qualifying_count += 1
                    new_this_query += 1
            print(f"[dental-p11] [{gi}] '{query}' -> {len(articles)} items, +{new_this_query} qualifying (total {qualifying_count})", flush=True)
            _touch()
        except Exception as e:
            print(f"[dental-p11] [{gi}] '{query}' ERROR: {e}", flush=True)
            _touch()
        time.sleep(0.6)
...
def main():
    ...
    print(f"[dental-p11] {len(queries)} queries total (global index shown), MIN_REVIEWS={MIN_REVIEWS}", flush=True)

    CHUNK = 30
    for start in range(0, len(queries), CHUNK):
        chunk = queries[start:start + CHUNK]
        print(f"[dental-p11] opening fresh session for global queries {start+1}-{start+len(chunk)}", flush=True)
        _touch()
        try:
            with StealthySession(headless=True, network_idle=False, max_pages=1) as session:
                run_batch(chunk, session, start + 1)
        except Exception as e:
            print(f"[dental-p11] session-level ERROR: {e}", flush=True)
            _touch()

    print("[dental-p11] DONE ALL", flush=True)
