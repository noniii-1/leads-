# -*- coding: utf-8 -*-
"""
Ninth pass: keyword variety is exhausted (pass 7-8 only added ~17 combined).
Lower MIN_REVIEWS to 10 (still requires the business to have a real
double-digit review base, so not a zero-footprint "de barrio" shop) and
re-run the original core keywords across all 33 comunas — this re-surfaces
businesses that were seen in earlier passes but discarded for having 10-14
reviews.
"""
import os
import time
...
def _watchdog():
    while True:
        ...
        if stale > WATCHDOG_SECS:
            print(f"[dental-p9] WATCHDOG: no progress for {stale:.0f}s, force-exiting", flush=True)
            os._exit(1)
        ...
        try:
            ...
            for a in articles:
                ...
                if rec["reviews"] >= MIN_REVIEWS and not rec["has_website"]:
                    ...
                    qualifying_count += 1
                    new_this_query += 1
            print(f"[dental-p9] [{gi}] '{query}' -> {len(articles)} items, +{new_this_query} qualifying (total {qualifying_count})", flush=True)
            _touch()
        except Exception as e:
            print(f"[dental-p9] [{gi}] '{query}' ERROR: {e}", flush=True)
            _touch()
        time.sleep(0.6)
...
def main():
    ...
    print(f"[dental-p9] {len(queries)} queries total (global index shown), watchdog={WATCHDOG_SECS}s, MIN_REVIEWS={MIN_REVIEWS}", flush=True)

    CHUNK = 30
    for start in range(0, len(queries), CHUNK):
        chunk = queries[start:start + CHUNK]
        print(f"[dental-p9] opening fresh session for global queries {start+1}-{start+len(chunk)}", flush=True)
        _touch()
        try:
            with StealthySession(headless=True, network_idle=False, max_pages=1) as session:
                run_batch(chunk, session, start + 1)
        except Exception as e:
            print(f"[dental-p9] session-level ERROR: {e}", flush=True)
            _touch()

    print("[dental-p9] DONE ALL", flush=True)
