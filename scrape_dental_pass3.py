# -*- coding: utf-8 -*-
"""
Third pass: fresh keyword variants + slightly lower MIN_REVIEWS (15, matching
the original project's own threshold), same 33 comunas. Appends to the same
dental_candidates.jsonl. Session restarts every 40 queries for resilience.
"""
import time
import json
...
KEYWORDS = [
    "salud dental clinica", "policlinico dental", "protesis dental",
    "periodoncia", "dentista particular", "blanqueamiento dental clinica",
]
        ...
        try:
            ...
            for a in articles:
                ...
                if rec["reviews"] >= MIN_REVIEWS and not rec["has_website"]:
                    ...
                    qualifying_count += 1
                    new_this_query += 1
            print(f"[dental-p3] [{qi}/{len(queries)}] '{query}' -> {len(articles)} items, +{new_this_query} qualifying (total {qualifying_count})", flush=True)
        except Exception as e:
            print(f"[dental-p3] [{qi}/{len(queries)}] '{query}' ERROR: {e}", flush=True)
        time.sleep(0.6)


def main():
    queries = build_queries()
    print(f"[dental-p3] {len(queries)} queries total across {len(COMUNAS)} comunas", flush=True)

    CHUNK = 40
    for start in range(0, len(queries), CHUNK):
        chunk = queries[start:start + CHUNK]
        print(f"[dental-p3] opening fresh session for queries {start+1}-{start+len(chunk)}", flush=True)
        try:
            with StealthySession(headless=True, network_idle=False, max_pages=1) as session:
                run_batch(chunk, session)
        except Exception as e:
            print(f"[dental-p3] session-level ERROR: {e}", flush=True)

    print("[dental-p3] DONE ALL", flush=True)
