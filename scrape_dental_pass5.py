# -*- coding: utf-8 -*-
"""
Fifth pass to close the gap to the 140 target. Same 33 comunas, fresh keywords.
"""
import time
import json
...
KEYWORDS = [
    "clinica dental integral", "centro dental familiar",
    "urgencias odontologicas", "dentista cerca de mi",
]
        ...
        try:
            ...
            for a in articles:
                ...
                if rec["reviews"] >= MIN_REVIEWS and not rec["has_website"]:
                    ...
                    qualifying_count += 1
                    new_this_query += 1
            print(f"[dental-p5] [{gi}] '{query}' -> {len(articles)} items, +{new_this_query} qualifying (total {qualifying_count})", flush=True)
        except Exception as e:
            print(f"[dental-p5] [{gi}] '{query}' ERROR: {e}", flush=True)
        time.sleep(0.6)


def main():
    queries = build_queries()
    print(f"[dental-p5] {len(queries)} queries total (global index shown)", flush=True)

    CHUNK = 40
    for start in range(0, len(queries), CHUNK):
        chunk = queries[start:start + CHUNK]
        print(f"[dental-p5] opening fresh session for global queries {start+1}-{start+len(chunk)}", flush=True)
        try:
            with StealthySession(headless=True, network_idle=False, max_pages=1) as session:
                run_batch(chunk, session, start + 1)
        except Exception as e:
            print(f"[dental-p5] session-level ERROR: {e}", flush=True)

    print("[dental-p5] DONE ALL", flush=True)
